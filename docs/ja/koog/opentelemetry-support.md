...
事前に設定された`OpenTelemetrySdk`インスタンスを注入します。

- `setSdk(sdk)`を呼び出すと、提供された`SDK`はそのまま使用され、`addSpanExporter`、`addSpanProcessor`、`addResourceAttributes`、または`setSampler`を介して適用されたカスタム設定はすべて無視されます。
- `トレーサー`の計装スコープ名/バージョンは、サービス情報と整合されます。

| Name | Data type         | Required | Description                           |
|:-----|:------------------|:---------|:--------------------------------------|
| `sdk`| `OpenTelemetrySdk`| Yes      | エージェントで使用する`SDK`インスタンス。 |
...
!!! note
    カスタムエクスポーターを設定しない場合、Koogはデフォルトでコンソール`LoggingSpanExporter`を使用します。これはローカル開発およびデバッグ時に役立ちます。
...
Langfuseは、LLM/エージェントのワークロードに対してトレースの視覚化と分析を提供します。

ヘルパー関数を使用して、Koogを構成し、OpenTelemetryトレースをLangfuseに直接エクスポートできます。
...
Langfuseとの統合に関する詳細なドキュメントについては、[こちら](opentelemetry-langfuse-exporter.md)を参照してください。

## W&B Weaveとの統合

W&B Weaveは、LLM/エージェントのワークロードに対してトレースの視覚化と分析を提供します。W&B Weaveとの統合は、事前定義されたエクスポーターを介して構成できます。
...
W&B Weaveとの統合に関する詳細なドキュメントについては、[こちら](opentelemetry-weave-exporter.md)を参照してください。
