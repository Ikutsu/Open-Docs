...
스팬이 내보내지기 전에 처리할 스팬 프로세서를 추가합니다. 다음 인수를 사용합니다:

| 이름        | 데이터 유형       | 필수 | 기본값 | 설명                                                                   |
|:------------|:----------------|:---|:-----|:-----------------------------------------------------------------------|
| `processor` | `SpanProcessor` | 예 |      | 내보내기 전에 텔레메트리 데이터를 처리하는 사용자 정의 로직을 포함하는 스팬 프로세서입니다. |
...
1.  **Jaeger 또는 Langfuse에 추적이 나타나지 않음**
